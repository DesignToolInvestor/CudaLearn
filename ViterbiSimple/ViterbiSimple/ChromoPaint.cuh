/*
  C h r o m o P a i n t U n P h a s e . c u h

    This function starts with a collection of chromosomes and paints each one inter terms of every
...
void ChromoPaintAllUnPhase(
  PeopleIndexT path[], 
  unsigned numPeople, unsigned numSnp, const uint8_t read[], const ProbT stateStayProb[],
  unsigned threadPerBlock, unsigned enoughThreads);

template<
  typename ProbT, typename PeopleIndexT, typename SnpIndexT>
void ChromoPaintUnPhaseFor(
  ProbT lattus[], 
  unsigned numPeople, unsigned numSnp, const uint8_t read[], const ProbT stateStayProb[],
...
template<
  typename ProbT, typename PeopleIndexT, typename SnpIndexT>
void ChromoPaintUnPhaseRev(
  PeopleIndexT path[],
  unsigned numPeople, unsigned numSnp, const ProbT lattus[], 
  unsigned toDoStart, unsigned toDoNum);
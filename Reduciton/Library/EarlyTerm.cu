...
// Do instantiations
// ToDo; Rationalize and clean up the instantiation process
template __global__ void AddReduceEarlyTerm(float* partSum, float* data, size_t dataSize);

template __global__ void AddReduceEarlyTerm(int* partSum, int* data, size_t dataSize);